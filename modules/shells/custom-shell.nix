# Custom (non-noctalia) wlroots shell built from rofi / wlogout / swaync / hyprlock / etc.

{ ... }:
{
  flake.homeModules.custom-shell =
    {
      config,
      lib,
      pkgs,
    ...
    in
    {
      ...
      config = lib.mkIf (config.smona.desktop.shell == "custom") {
        ...
        programs.rofi = {
          ...
          theme = ../../desktops/wlroots/rofi-theme.rasi;
        };
  ...
  flake.nixosModules.custom-shell =
    { config, lib, ... }:
    {
      config = lib.mkIf (config.smona.desktop.shell == "custom") {
